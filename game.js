...
function spawnHunter(forceSmall){
  ...
  const size=baseSize*(1+diff()*0.8); // Scale enemy size with difficulty (80% max)
...
function spawnSpecialEnemy(){
  ...
  const sizeMultiplier=1+diff()*0.8; // Scale special enemy size with difficulty (80% max)
...
function draw(ts){
  ...
  if(energy >= 25){
    ...
    ctx.beginPath();ctx.arc(dcX,dcY,dcR,0,Math.PI*2,false);ctx.stroke();
  ...
  for(let i=orbs.length-1;i>=0;i--){
    ...
    if(dist2(player.x,player.y,o.x,o.y)<(player.displayR+o.r*.5)**2){
      ...
      if(o.golden){ energy += 20; energy = Math.min(energy, maxEnergy); floatText(o.x, o.y - o.r - 8, `+20 ENERGY`, 200); }
    ...
    for(let i=hunters.length-1;i>=0;i--){
      ...
      if(dist2(player.x,player.y,h.x,h.y)<colD*colD){
        if(player.displayR>h.r*1.0){
          ...
          const energyGain = Math.floor(h.r * 2 * (1 + level * 0.02)); // Energy from eating enemies
          if(h.type==='splitter'){
          ...
          if(energyGain > 0) floatText(h.x, h.y - h.r - 25, `+${energyGain} ENERGY`, 200); // Show energy gain
          score+=pts;xp+=pts;player.r=Math.min(75,player.r+h.r*.1);
          energy += energyGain; energy = Math.min(energy, maxEnergy); // Add energy gain
          while(xp>=xpNext&&level<MAX_LEVEL){
            xp-=xpNext;xpNext=Math.floor(xpNext*1.2);level++;showLevelUp();
            burst(player.x,player.y,player.hue,20,true);
...
canvas.addEventListener('touchstart',e=>{
  ...
  if(now-lastTap<400&&energy>=25){
    // Burst effect: damage and knockback nearby enemies, convert some to orbs
    let enemiesConverted = 0;
    let enemiesDamaged = 0;
    for(let i=hunters.length-1;i>=0;i--){
      const h = hunters[i];
      const d2 = dist2(player.x, player.y, h.x, h.y);
      if(d2 < 200*200){ // Within 200 units
        if(enemiesConverted < 3) { // Convert up to 3 enemies to orbs
          // Create orb at enemy position
          const orbValue = Math.max(1, Math.floor(h.r * 2));
          orbs.push({
            x: h.x, y: h.y,
            r: Math.max(4, h.r * 0.8),
            hue: h.hue,
            pulse: rnd(0, Math.PI*2), ps: rnd(.02,.05),
            vx: rnd(-.4,.4), vy: rnd(-.4,.4),
            value: orbValue,
            golden: false, big: false,
          });
          // Remove the enemy
          hunters.splice(i,1);
          enemiesConverted++;
        } else {
          // Damage and knockback remaining enemies
          const damage = Math.floor(25 + level * 2); // More damage at higher levels
          h.hp -= damage;
          if(h.hp <= 0){
            burst(h.x, h.y, h.hue, 12, false);
            floatText(h.x, h.y - h.r - 10, `DESTROYED`, 0);
            hunters.splice(i,1);
            spawnHunter();
          } else {
            ...
            h.vx += (dx/dist) * 8; // Push away from player
            h.vy += (dy/dist) * 8;
            burst(h.x, h.y, h.hue, 8, false);
            floatText(h.x, h.y - h.r - 10, `-${damage} HP`, 0);
            enemiesDamaged++;
          }
        }
      }
    ...
    energy -= 25;
    updateHUD();
    burst(player.x, player.y, player.hue, 25, true); // Even bigger burst effect
    shake(15);
    let message = '';
    if(enemiesConverted > 0) message += `${enemiesConverted} TO ORBS`;
    if(enemiesDamaged > 0) {
      if(message) message += `, ${enemiesDamaged} DAMAGED`;
      else message += `${enemiesDamaged} DAMAGED`;
    }
    if(message) floatText(player.x, player.y, message, player.hue);
  }
  lastTap=now;
},{passive:true});
